...
[特性](#特性) · [快速开始](#快速开始) · [动态演示](#动态演示) · [应用场景](#应用场景) · [功能一览](#功能一览) · [内置 Agent](#内置-agent-助手) · [部署](#部署) · [技术架构](#技术架构) · [参与贡献](#参与贡献) · [许可](#许可)
...
<p align="center">
  <img src="docs/screenshots/demo-agent-nl.gif" alt="灵犀工作台 · Agent 自然语言操作实测" width="860">
</p>

> 用口语直接指挥工作台：说一句「帮我建个任务，周五前交」，Agent 通过大模型 function-calling 理解意图、填好参数并生成确认按钮，你点一下才真正落库——不必在一堆表单里点来点去。
...
- 👥 **多用户协作**：注册即成为管理员 → 建项目 → 邀请成员 → 分配角色（负责人 / 可编辑 / 只读）；任务分配与评论触发站内通知；项目负责人可从卡片右上角一键归档或删除项目。
- 💡 **团队工具箱**：想法投票、SWOT / 精益等 7 种画布、客户管理、回顾复盘一应俱全。
- 🤖 **内置 Agent 助手**：一键周报、逾期扫描、跨实体联动分析（健康诊断 / 依赖链 / 负载 / 风险雷达），并支持**自然语言对话驱动操作**——可对接 LLM，说人话就能建任务、改状态、查进度。
...
| 📁 项目 | `?v=projects` | `js/views.js` | 项目管理主页。项目卡片网格，含进度条、状态（进行中 / 已归档）、颜色标签；卡片右上角常显编辑 / 删除图标（仅对项目负责人可见），可新建 / 编辑 / 归档 / 删除项目，进入项目详情邀请成员并设置角色（负责人 / 可编辑 / 只读）。 |
...
- **自然语言对话**：直接说"这周谁最忙""帮我建个任务，周五前交""把登录 bug 挪到这周并提醒老王"，Agent 会做意图识别、多轮追问补全参数、写操作二次确认、并按权限执行
- **悬浮窗入口**：右下角悬浮球，点击展开即可对话，不占用主界面
- **对接大模型（自然语言操作的核心）**：在「Agent → 配置与日志」填入 DeepSeek / OpenAI / 自定义兼容 API 的 Key 与模型后即可启用。启用后，Agent 通过 **function-calling** 真正理解你的口语化指令：

  - 大模型负责"听懂话 + 选工具 + 填参数"，后端工具注册表负责"读数据 / 改数据 / 查权限"
  - 上下文里**只暴露当前用户「可写」的项目**，避免 Agent 把任务误挂到你没权限的项目上；若没有可写项目，任务会创建为「未分配」状态，保证"说建就建"
  - **read / safe 级工具**（如列出逾期、发送通知）直接执行
  - **write 级工具**（创建 / 移动 / 更新任务）只生成确认按钮，**你点击后才生效**——防止模型误改，安全不变
...
│   ├── agent.js            # Agent 引擎（工具注册表 + 技能库 + cron 调度 + LLM function-calling）
...
| POST | `/api/agent/chat` | 自然语言对话（对接 LLM 时走 function-calling） |
...
- [x] 补充界面截图（9 张）与动态演示（GIF ×2：总览 + 自然语言操作）
- [ ] 补充短视频演示（操作教程类）
- [ ] WebDAV / 网盘自动备份
- [ ] 实时协作（WebSocket 通知）
- [ ] 移动端 PWA 适配
